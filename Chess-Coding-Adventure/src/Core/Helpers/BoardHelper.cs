...
public static class BoardHelper
{
	...
	public static readonly Coord[] RookDirections = [new(-1, 0), new(1, 0), new(0, 1), new(0, -1)];
	public static readonly Coord[] BishopDirections = [new(-1, 1), new(1, 1), new(1, -1), new(-1, -1)];
	...
	{
		return new(FileIndex(squareIndex), RankIndex(squareIndex));
	}
	...
	public static int SquareIndexFromName(string name)
	{
		var fileName = name[0];
		var rankName = name[1];
		var fileIndex = fileNames.IndexOf(fileName);
		var rankIndex = rankNames.IndexOf(rankName);
		return IndexFromCoord(fileIndex, rankIndex);
	}

	public static bool IsValidCoordinate(int x, int y) => x is >=0 and <8 && y is >=0 and <8;
	...
	public static string CreateDiagram(Board board, bool blackAtTop = true, bool includeFen = true, bool includeZobristKey = true)
	{
		...
		var lastMoveSquare = board.AllGameMoves.Count > 0 ? board.AllGameMoves[^1].TargetSquare : -1;
		...
		for (var y = 0; y < 8; y++)
		{
			...
			for (var x = 0; x < 8; x++)
			{
				...
				var squareIndex = IndexFromCoord(fileIndex, rankIndex);
