...
public static class Bits
{
	...
	static Bits()
	{
		...
		{
			FileMask[i] = FileA << i;
			var left = i > 0 ? FileA << (i - 1) : 0;
			var right = i < 7 ? FileA << (i + 1) : 0;
		...
		for (var square = 0; square < 64; square++)
		{
			...
			var adjacentFiles = FileA << Max(0, file - 1) | FileA << Min(7, file + 1);
			// Passed pawn mask
			var whiteForwardMask = ~(ulong.MaxValue >> (64 - 8 * (rank + 1)));
			var blackForwardMask = ((1ul << 8 * rank) - 1);
			...
			// Pawn support mask
			var adjacent = (1ul << (square - 1) | 1ul << (square + 1)) & adjacentFiles;
