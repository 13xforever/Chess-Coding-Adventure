	...
	static BitBoardUtility()
	{
		...
		void ProcessSquare(int x, int y)
		{
			...
			for (var dirIndex = 0; dirIndex < 4; dirIndex++)
			{
				...
				{
					var orthoX = x + orthoDir[dirIndex].x * dst;
					var orthoY = y + orthoDir[dirIndex].y * dst;
					var diagX = x + diagDir[dirIndex].x * dst;
					var diagY = y + diagDir[dirIndex].y * dst;

					if (ValidSquareIndex(orthoX, orthoY, out var orthoTargetIndex))
					{
						if (dst == 1)
						{
					...
					if (ValidSquareIndex(diagX, diagY, out var diagTargetIndex))
					{
						if (dst == 1)
						{
				...
				{
					var knightX = x + knightJumps[i].x;
					var knightY = y + knightJumps[i].y;
					if (ValidSquareIndex(knightX, knightY, out var knightTargetSquare))
					{
				...
				if (ValidSquareIndex(x + 1, y + 1, out var whitePawnRight))
				{
					WhitePawnAttacks[squareIndex] |= 1ul << whitePawnRight;
				}
				if (ValidSquareIndex(x - 1, y + 1, out var whitePawnLeft))
				{
					WhitePawnAttacks[squareIndex] |= 1ul << whitePawnLeft;
				}


				if (ValidSquareIndex(x + 1, y - 1, out var blackPawnAttackRight))
				{
					BlackPawnAttacks[squareIndex] |= 1ul << blackPawnAttackRight;
				}
				if (ValidSquareIndex(x - 1, y - 1, out var blackPawnAttackLeft))
				{
